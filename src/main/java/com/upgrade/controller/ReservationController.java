...
public class ReservationController {
	...
	public ResponseEntity<ReservationDto> getReservation(@PathVariable("identifier") String identifier,
		...
		URI locationUri = ucb.path(Constants.ROOT_API).path("/").path(String.valueOf(dto.getIdentifier())).build()
				.toUri();
		hdrs.setLocation(locationUri);
		hdrs.setETag(Long.toString(reservation.getVersion()));
	...
	public synchronized ResponseEntity<Reservation> addBooking(@RequestBody ReservationDto res, UriComponentsBuilder ucb)
		...
		HttpHeaders hdrs = createHeaders(ucb, reserv);
	...
	public synchronized ResponseEntity<Reservation> modifyBooking(WebRequest request, @PathVariable("identifier") String identifier, @RequestBody ReservationDto res,
		...
		Reservation reserv =  reservationService.getReservationByIdentifier(identifier);
		if (null == reserv) {
		...
		if (!Long.valueOf(ifMatchValue.replace("\"", "")).equals(reserv.getVersion()) ){
		...
		ResponseEntity<Reservation> responseEntity = new ResponseEntity<Reservation>(reserv, hdrs, HttpStatus.OK);
	...
	public synchronized ResponseEntity<ReservationDto> deleteBooking( WebRequest request, @PathVariable("identifier") String identifier,
		...
		try {
			...
			log.info("ValidationException when deleting a reservation:[{}]", e.getMessage());
			throw new ValidationException(e.getMessage());
		}
	...
	private HttpHeaders createHeaders(UriComponentsBuilder ucb, Reservation reserv) {

		HttpHeaders hdrs = new HttpHeaders();
		URI locationUri = ucb.path(Constants.ROOT_API).path("/").path(String.valueOf(reserv.getIdentifier())).build().toUri();
		hdrs.setLocation(locationUri);
		hdrs.setETag("\""+Long.toString(reserv.getVersion())+"\"");
		return hdrs;
	}
